<div class="prose">
	<h2>controls</h2>
	<div class="group">
		<div>
			<strong>queue a move</strong>: <code>tap</code> or <code>click</code> the environment
		</div>
		<div>
			or press
			<code>arrow keys</code>, <code>wasd</code>, <code>hjkl</code>
		</div>
	</div>
	<div><strong>move and end turn</strong>: hold <code>shift</code> and move</div>
	<div><strong>toggle clock</strong>: <code>Backtick `</code></div>
	<div><strong>take one turn</strong>: <code>1</code></div>
	<div><strong>delete all saved data</strong>: <code>0</code></div>
	<div><strong>toggle song</strong>: <code>=</code></div>
	<div><strong>restart</strong>: <code>r</code> or <code>g</code></div>
</div>

<style>
	.prose {
		font-size: var(--size_lg);
	}
	strong {
		color: var(--brown_text_color);
	}
	.group {
		display: flex;
		flex-direction: column;
		align-items: flex-end;
	}
</style>
